...
"""
from positive_inside_analysis import parse_prd, grade_segment
...
def main():
    ...
    for prd in prd_list:
        ...
        # print results
        # break
        wgp_best, sec_wgp = parse_prd(prd)
        if wgp_best is None:
            continue
        KR_diff = KR_ddG(wgp_best, polyval) - KR_ddG(sec_wgp, polyval)
        if prd_pars['pred_correct']:
            correct_KR.append(KR_diff)
        else:
            wrong_KR.append(KR_diff)
