...
for roc in file_list:
    ...
    cont = f.read().split('\n')
    f.close()
    res = {'pdbtm': {'Qok': 0, 'topo': 0}, 'opm': {'Qok': 0, 'topo': 0}}
    for line in cont:
        split = line.split()
        if len(split) < 2:
            continue
        ...
        if split[0] == 'Q_ok':
            res[tech]['Qok'] = split[1]
        if split[1] == 'correct':
           res[tech]['topo_percent'] = float(split[5])
    res['Qok_pdbtm'] = res['pdbtm']['Qok']
    res['Qok_opm'] = res['opm']['Qok']
    res['topo_percent_pdbtm'] = res['pdbtm']['topo_percent']
    res['topo_percent_opm'] = res['opm']['topo_percent']
